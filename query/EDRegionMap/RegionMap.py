...
import sys
import json
from EDRegionMap.RegionMapData import regions, regionmap
...
def main():
    sysname = sys.argv[1]
    url = 'https://www.edsm.net/api-v1/systems?systemName=' + \
        urllib.parse.quote(sysname) + '&coords=1&showId=1'
    with urllib.request.urlopen(url) as f:
        systems = json.load(f)
    for system in systems:
        region = findRegion(system['coords']['x'],
                            system['coords']['y'], system['coords']['z'])
        ...
        x = (((id64 >> (30 - masscode * 2)) &
              (0x3FFF >> masscode)) << masscode) * 10 + x0
